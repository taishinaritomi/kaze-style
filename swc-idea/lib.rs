use ast_node::node_to_expr;
use config::{parse_config, InputCollector, InputConfig, InputImport};
use swc_core::{
  common::DUMMY_SP,
  ecma::{
    ast::{
...
impl TransformVisitor {
  fn new(input_config: InputConfig) -> Self {
    let transforms = input_config
      ...
      .map(|input_transform| Transform {
        ...
        to: input_transform.pre.to_string(),
    ...
    Self {
      ...
      collector: input_config.collector,
      imports: input_config.imports,
      ...
      module.body.insert(
        ...
        ModuleItem::ModuleDecl(ModuleDecl::Import(ImportDecl {
          ...
          src: Box::new(Str {
            raw: None,
            span: DUMMY_SP,
            value: import.source.to_string().into(),
          }),
          type_only: false,
          specifiers: vec![ImportSpecifier::Named(ImportNamedSpecifier {
            ...
            local: Ident {
              span: DUMMY_SP,
              sym: import.specifier.to_string().into(),
              optional: false,
            },
            span: DUMMY_SP,
          })],
  ...
  fn module_item_collector(&mut self, module_item: &mut ModuleItem) {
    match &module_item {
            ...
            for transform in self.transforms.iter() {
              ...
              if is_target == true {
                let ident = Ident {
                  optional: false,
                  span: DUMMY_SP,
                  sym: format!("__{}", self.ident_index).to_string().into(),
                };
  ...
  fn var_decl_collector(&mut self, var_decl: &mut VarDecl) {
          ...
          decls: vec![VarDeclarator {
            ...
            init: Some(Box::new(Expr::Call(CallExpr {
              ...
              callee: Callee::Expr(Box::new(Expr::Ident(Ident {
                span: DUMMY_SP,
                sym: self.collector.specifier.to_string().into(),
                optional: false,
              }))),
            ...
            name: Pat::Ident(BindingIdent {
              id: Ident {
                optional: false,
                span: DUMMY_SP,
                sym: self.collector.export_name.to_string().into(),
              },
              type_ann: None,
            }),
          }],
...
impl VisitMut for TransformVisitor {
  ...
  fn visit_mut_module_item(&mut self, module_item: &mut ModuleItem) {
    ...
    self.module_item_collector(module_item);
  }

  fn visit_mut_var_decl(&mut self, var_decl: &mut VarDecl) {
    var_decl.visit_mut_children_with(self);
    self.var_decl_collector(var_decl);
  }
...
mod tests {
  ...
  test!(
    ...
    // Output codes after
    r#"
    import { __preTarget as target } from 'target_source';
    const x1 = target({}, { "filename": "filename.ts", "build": for_build }, 0);
    const __0 = target({}, { "filename": "filename.ts", "build": for_build }, 1);
    export const __collector = collector([ x1, __0 ]);
    "#
  );
  ...
  test!(
    ...
    // Output codes after
    r#"
    import { __preTarget as _target1 } from 'target_source';
    import { __preTarget as _target2 } from 'target_source';
    const x1 = _target1({}, { "filename": "filename.ts", "build": for_build }, 0);
    const x2 = _target2({}, { "filename": "filename.ts", "build": for_build }, 1);
    export const __collector = collector([ x1, x2 ]);
    "#
  );
  ...
  test!(
    ...
    import * as namespace1 from 'target_source';
    import * as namespace2 from 'target_source';
    const x1 = namespace1.__preTarget({}, { "filename": "filename.ts", "build": for_build }, 0);
    const x2 = namespace2.__preTarget({}, { "filename": "filename.ts", "build": for_build }, 1);
    export const __collector = collector([ x1, x2 ]);
    "#
  );
  test!(
    ...
    // Output codes after
    r#"
    import { __preTarget as target, __preTarget as _target } from 'target_source';
    import { __preTarget as __target } from 'target_source';
    import * as namespace from 'target_source';
    const x1 = target({}, { "filename": "filename.ts", "build": for_build }, 0);
    const x2 = _target({}, { "filename": "filename.ts", "build": for_build }, 1);
    const x3 = __target({}, { "filename": "filename.ts", "build": for_build }, 2);
    const x4 = namespace.__preTarget({}, { "filename": "filename.ts", "build": for_build }, 3);
    const __0 = namespace.__preTarget({}, { "filename": "filename.ts", "build": for_build }, 4);
    const x5 = namespace.dummy({});
    export const __collector = collector([ x1, x2, x3, x4, __0 ]);
    "#
  );
  test!(
    Default::default(),
